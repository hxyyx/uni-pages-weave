...
export function assertDeniedKeys(
  value: Record<string, unknown>,
  deniedKeys: readonly string[],
  fieldPath: string,
): void {
  const denied = new Set(deniedKeys);
  const unsupportedKeys = Object.keys(value).filter((key) => denied.has(key));

  if (unsupportedKeys.length > 0) {
    throw new Error(`${fieldPath} contains unsupported keys: ${unsupportedKeys.join(', ')}.`);
  }
}

export function assertUnpatchableKeys(
  value: Record<string, unknown>,
  unpatchableKeys: readonly string[],
  fieldPath: string,
): void {
  const unpatchable = new Set(unpatchableKeys);
  const keys = Object.keys(value).filter((key) => unpatchable.has(key));

  if (keys.length > 0) {
    throw new Error(`${fieldPath} cannot patch keys: ${keys.join(', ')}.`);
  }
}

export function validatePatchTargetKeys(
  patch: ConditionalPatch | ConditionalChildPatch,
  fieldPath: string,
  options: {
    deniedKeys?: readonly string[];
    unpatchableKeys?: readonly string[];
  ...
  assertDeniedKeys(patch.patch, options.deniedKeys ?? [], fieldPath);
  assertUnpatchableKeys(patch.patch, options.unpatchableKeys ?? [], fieldPath);
}
