...
| 路径                      | 作用                                                                                    |
| ------------------------- | --------------------------------------------------------------------------------------- |
| `app.upw.json`            | 应用级配置，保存 `homePath`、`globalStyle`、`tabBar`、`easycom`、`subpackages` 等字段。 |
...
- `app.upw.json` 中的分包字段统一使用小写 `subpackages`，不要使用 uni-app 输出侧的 `subPackages`。
...
| 字段         | 类型               | 必填 | 说明                                                                                   |
| ------------ | ------------------ | ---- | -------------------------------------------------------------------------------------- |
| `when`       | `string[]`         | 否   | 简单 `#ifdef` 条件。数组内多个平台是或关系，例如 `["app-plus", "h5"]` 会输出 `APP-PLUS |     | H5`。 |
...
`app.upw.json` 保存应用级配置。首页由 `homePath` 指定，分包声明在 `subpackages` 中。
...
| 字段            | 类型           | 必填 | 说明                                                                             |
| --------------- | -------------- | ---- | -------------------------------------------------------------------------------- |
| `$upw`          | `AppMeta`      | 否   | 应用级 upw 扩展配置，目前用于声明应用级 `patches`。                              |
| `homePath`      | `string`       | 是   | 应用首页路径，必须指向主包页面。                                                 |
| `subpackages`   | `Subpackage[]` | 否   | 分包配置，统一使用小写 `subpackages`。                                           |
...
应用级 patch 只能修改已存在的顶层对象字段，例如 `globalStyle`。不能按条件新增顶层属性，也不能按条件整体替换顶层非对象属性。
